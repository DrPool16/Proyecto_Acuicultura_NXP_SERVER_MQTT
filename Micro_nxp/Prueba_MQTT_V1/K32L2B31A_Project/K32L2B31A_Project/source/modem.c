...
void Modem_Task_Run(void){
	...
	case ST_MOD_OPEN_MQTT:
		Modem_Send_Cmd("AT+QMTOPEN=0,\"142.93.88.99\",1883\r\n"); //tx "AT+QMTOPEN=0,"142.93.88.99",1883"
...
void Modem_Check_URC_Run(void){
	...
	if(Respuesta_Modem("+QMTRECV")){ // mensaje MQTT
		...
		if(Test_Rta_Modem("CMTI")){ // recibio un mensaje de Texto
			// cmdRxViaSMS = 1;
			// mirar el resto del buffer
			// AT+GSN ?
			// AT+CSQ ?
			// AT+CCLK ? Modem_Send_Cmd("AT+CCLK\r\n");
			//           3 segs de espera
			//			 if(sms){
			//				 Enviar UN SMS con la Rta del Modem
			//			 }
			//			 la rta del modem esta en buffer_comando_recibido
			//			 >
			//			 modemSt = PUB
			//			 Modem_Send_Cmd(buffer_comando_recibido)
			//			 CTL_Z
		}
	}
}
